...
엘리먼트를 블록 레벨의 플렉스 컨테이너로 표시하는 방법입니다.
...
> flexBox 의 호환성은 99%의 높은 호환성을 보이나 IE10이나 이하에서는 지원하지 않습니다.
...
flex-direction 을 column 으로 지정했으니 기본값 가로정렬이 아닌 열(column)을 따라 세로 순서대로 위쪽부터 배치됩니다.
...
실무에서 가로세로 중앙 정렬을 위해 가장 많이 사용하는 스타일링입니다. 
```html{3-8}
<style>
.flex-container {
  display: flex;
...
아이템 (주로 레이아웃을 감싼 레이아웃 wrapper) 을 중앙에 둡니다.
